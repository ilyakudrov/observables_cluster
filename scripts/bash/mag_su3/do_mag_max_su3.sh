...
echo --ftype ${conf_format} --fbasename ${conf_path_start} \
 ...
 --output_SA_functional ${path_functional_output} --gaugecopies 1 --output_conf "${conf_path_output}/conf.SP_" --output_ending ${conf_path_end} --seed ${seed} --microupdates 6 --precision ${tolerance}

/home/clusters/rrcmpi/kudrov/SU3_MA_gauge_GPU/src/gaugefixing/apps/MAGaugeFixingSU3_maximization_4D_DP_N${L_spat}T${L_time}  --ftype ${conf_format} --fbasename ${conf_path_start} \
 --fending "$conf_path_end" --reinterpret DOUBLE --fnumberformat ${padding} -m 1 --fstartnumber ${i} --sasteps 5 --samin 2.5 --samax 0.5\
 --output_SA_functional ${path_functional_output} --gaugecopies 1 --output_conf "${conf_path_output}/conf.SP_" --output_ending "${conf_path_end}" --seed ${seed} --microupdates 6 --precision ${tolerance}
