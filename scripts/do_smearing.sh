...
if [ -f ${path_conf_wilson1} ] && [ -s ${path_conf_wilson1} ] ; then

mkdir -p ${path_conf_wilson_loop}
...
parameters="-conf_format_wilson ${conf_format_wilson} -conf_path_wilson ${conf_path_wilson1} -bytes_skip_wilson ${bytes_skip_wilson}\
    ...
    -path_wilson ${path_wilson}/wilson_loop_`printf %0${padding_wilson}d $i` -path_flux ${path_flux}/flux_tube_`printf %0${padding_wilson}d $i`\
