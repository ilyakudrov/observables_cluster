...
parameters="-conf_format ${conf_format} -path_conf $path_conf -conf_format ${conf_format} -bytes_skip ${bites_skip} \
    -path_conf_monopole ${path_conf_monopole1} -path_conf_monopoless ${path_conf_monopoless1}\
    -x_size ${L_spat} -y_size ${L_spat} -z_size ${L_spat} -t_size ${L_time}"

/home/clusters/rrcmpi/kudrov/general_code/apps/monopole_decomposition_su3/monopole_unitarity_${arch} $parameters
