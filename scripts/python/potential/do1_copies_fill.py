...
for beta, mu, conf_size, additional_parameters, decomposition_type, smearing_param in itertools.product(*iter_arrays):
    ...
    base_path_out = '/home/clusters/rrcmpi/kudrov/observables/data'
    path_params = f'{smearing}/{operator_type}/{representation}/{axis}\
                    /{theory_type}/{conf_type}/{conf_size}/{beta}/{mu}/{decomposition_type}\
                    /{smearing_param}/{additional_parameters}'
    parameters = f'--base_path {base_path} --base_path_out {base_path_out} --path_params {path_params} --copies {gauge_copies}'
    bashCommand = f'qsub -q long -v parameters=\"${parameters}\"'\
        f' -o {log_path}/{decomposition_type}.o -e {log_path}/{decomposition_type}.e ../../bash/potential/do_copies_fill.sh'
